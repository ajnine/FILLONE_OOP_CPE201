...
namespace FILLONE_OOP_CPE201
{
    internal class pos_dbconnection
    {
        ...
        {
            pos_sql = "SELECT * FROM pos_nameTbl INNER JOIN pos_picTBbl ON pos_nameTbl.pos_id = pos_picTBbl.pos_id INNER JOIN pos_priceTbl ON pos_picTBbl.pos_id = pos_priceTbl.pos_id";
        }

        public void pos_select_cashier()
        {
            pos_sql = "SELECT * FROM pos_nameTbl INNER JOIN pos_picTBbl ON pos_nameTbl.pos_id = pos_picTBbl.pos_id INNER JOIN pos_priceTbl ON pos_picTBbl.pos_id = pos_priceTbl.pos_id WHERE pos_nameTbl.pos_id =1";
        }

        public void pos_select_cashier1()
        {
            pos_sql = "SELECT * FROM pos_nameTbl INNER JOIN pos_picTBbl ON pos_nameTbl.pos_id = pos_picTBbl.pos_id INNER JOIN pos_priceTbl ON pos_picTBbl.pos_id = pos_priceTbl.pos_id WHERE pos_nameTbl.pos_id =2";
        }
